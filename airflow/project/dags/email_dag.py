...
with DAG(dag_id="email_dag",
    ...
    email_function = PythonOperator(
        task_id="email_function",
        python_callable=first_function,
    )

    email_function1 = PythonOperator(
        task_id="email_function1",
        python_callable=first_function1,
    )

    email_func2 = EmailOperator(
        task_id='send_email',
        to='zaynh162@gmail.com',
        subject='Airflow Alert',
        html_content=""" <h3>Email Test Airflow </h3> """,
    )

email_function >> email_func2>>email_function1
